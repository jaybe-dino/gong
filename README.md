...
- **테스트 71개** — 로직 41 · 통합 19(실제 Postgres) · E2E 11(Playwright).
...
**부재중 자동응답은 답장이 아니다.** `-5 부재중`으로 분류하면 시퀀스를 끝내지 않고
`PAUSED_OOO` 로 두고 본문에서 복귀일을 뽑아 그날 09시로 재스케줄한다.
...
src/
  lib/
    ...
    actions.ts          서버 액션
  app/                  12개 화면
scripts/                setup-db.mjs · seed.mjs · seed-data.mjs
tests/                  logic(41) · pipeline(19) · e2e(11)
samples/                소스별 CSV 예시
```
...
- **Gmail API · 인스타그램 실제 전송은 연결돼 있지 않다.** 발송은 `message` 와 `gate_block` 에
  기록되고 작업 큐가 생성되는 데까지 동작한다. 외부 전송 어댑터를 끼울 지점은
  `src/lib/actions.ts` 의 `startSend` 다
