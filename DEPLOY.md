...
그래서 앱 자신이 스키마를 적재한다. **터미널이 필요 없다.**
...
`CRON_SECRET` 을 붙여넣고 버튼 두 개를 순서대로 누른다.

1. **스키마 적용** — 테이블 34개 + 정책·템플릿 시드
2. **데모 데이터 넣기** — 크리에이터 1,742명 · 공구 5,258건

이 화면은 `Shell` 을 쓰지 않는다. Shell 이 담당자와 미확인 이벤트를 DB 에서
읽어서, 초기화 전에는 화면 자체가 뜨지 못한다.

현재 상태(연결 여부 · 테이블 수 · 크리에이터 수)를 위에 보여주고, 스키마가
없으면 2번 버튼을 비활성으로 잠근다. 시크릿은 폼으로만 오가고 URL 에 남지 않는다.
...
화면이든 API 든 **두 번 눌러도 안전하다.** 이미 있으면 손대지 않는다.
...
| `POST /api/admin/seed?force=1` · 화면의 체크박스 | 데이터 테이블 TRUNCATE 후 재적재 (정책 시드는 유지) |

시드 없이 빈 DB로 운영하려면 1번만 한다. 정책·템플릿·파이프라인 단계는
`002_seed_policy.sql` 에 들어 있어서 스키마 적용만으로 앱이 동작한다.
